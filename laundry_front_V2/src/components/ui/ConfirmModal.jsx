...
const ConfirmModal = ({
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-end sm:items-center justify-center animate-fade-in">
      ...
      {/* MODAL CONTENT */}
      <div className={`relative bg-white w-full max-w-md overflow-hidden animate-in slide-in-from-bottom sm:zoom-in-95 duration-300 shadow-2xl
        rounded-t-3xl sm:rounded-2xl border-t sm:border border-border/60`}>

        {/* MOBILE HANDLE */}
        <div className="w-12 h-1.5 bg-gray-200 rounded-full mx-auto mt-3 mb-1 sm:hidden"></div>
        ...
        <div className="p-6 sm:p-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-4">
            <button
              onClick={onClose}
              className="flex-1 px-6 py-3.5 bg-gray-50 text-text-secondary font-bold text-sm hover:bg-gray-100 rounded-xl transition-all active:scale-95 border border-border/50"
            >
              Annuler
            </button>
            <button
        ...
        <button 
          onClick={onClose}
          className="absolute top-4 end-4 p-2 text-text-muted hover:text-text-primary hover:bg-gray-100 rounded-lg transition-colors hidden sm:block"
        >
          <X size={20} />
        </button>
      </div>

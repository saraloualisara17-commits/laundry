  ...
  return (
    <div className="max-w-4xl mx-auto pb-12 space-y-6 animate-fade-in text-start px-4 md:px-0">
      ...
      <div className="flex items-center justify-between">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 px-4 py-2 bg-surface border border-border/50 rounded-xl text-[10px] font-black uppercase tracking-widest text-text-muted hover:text-text-primary transition-all active:scale-95 shadow-sm">
            ...
            return (
              <div key={i} className={`bg-background/50 rounded-2xl p-4 border border-border/50 flex items-start gap-4 ${row.fullWidth ? 'sm:col-span-2 lg:col-span-1' : ''}`}>
                ...
                <div className="min-w-0">
                  <p className="text-[8px] sm:text-[9px] font-black text-text-muted uppercase tracking-widest mb-0.5 sm:mb-1">{row.label}</p>
                  {row.isLink ? (

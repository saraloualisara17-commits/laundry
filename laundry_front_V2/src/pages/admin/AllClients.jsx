  ...
  return (
    <div className="space-y-6 pb-12 animate-fade-in text-start">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button onClick={loadData} className="flex items-center justify-center gap-2 px-5 py-2.5 bg-surface border border-border/50 text-text-primary rounded-xl text-xs font-black uppercase tracking-widest hover:bg-background transition-all shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        {[
          ...
          <div key={i} className="bg-surface p-5 rounded-[2rem] border border-border/50 shadow-sm flex items-center gap-5 transition-all hover:shadow-md">
            ...
            <div>
              <p className="text-xs font-bold text-text-muted uppercase tracking-widest mb-0.5">{stat.label}</p>
              <p className="text-2xl font-black text-text-primary tracking-tight">{stat.value}</p>
              {stat.sub && <p className="text-xs font-bold text-green-600 uppercase tracking-tighter mt-0.5">{stat.sub}</p>}
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-surface rounded-[2rem] border border-border/50 shadow-card overflow-hidden">
          ...
          <table className="w-full text-start">
            <thead>
              <tr className="bg-background/50 border-b border-border/50">
                <th className="px-8 py-3 sm:py-4 text-xs font-black text-text-muted uppercase tracking-[0.2em]">{t('admin.clients.table.client')}</th>
                <th className="px-8 py-3 sm:py-4 text-xs font-black text-text-muted uppercase tracking-[0.2em]">{t('admin.clients.table.contact')}</th>
                <th className="px-8 py-3 sm:py-4 text-xs font-black text-text-muted uppercase tracking-[0.2em]">{t('admin.clients.table.last_order')}</th>
                <th className="px-8 py-3 sm:py-4 text-xs font-black text-text-muted uppercase tracking-[0.2em] text-center">{t('admin.clients.table.orders')}</th>
                <th className="px-8 py-3 sm:py-4"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border/40">
              {loading && paginatedClients.length === 0 ? (
                <tr><td colSpan="5" className="py-20 text-center"><Loader2 size={32} className="animate-spin text-primary-500 mx-auto mb-4" /><p className="text-xs font-black uppercase tracking-widest opacity-40">{t('admin.clients.loading_db')}</p></td></tr>
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-start">
                          ...
                          <p className="text-xs text-text-muted font-bold uppercase tracking-widest mt-0.5">{t('admin.pro_ui.member_since')} {new Date(client.createdAt).toLocaleDateString('fr-FR', { year: 'numeric', month: 'short' })}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                      <div className="flex flex-col gap-1">
                        ...
                        {client.email && <div className="flex items-center gap-2 text-xs font-bold text-text-muted opacity-60"><Mail size={10} /> {client.email}</div>}
                      </div>
                    </td>
                    <td className="px-8 py-5">
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-start">
                    ...
                    <p className="text-xs text-text-muted font-bold uppercase tracking-widest">{t('admin.pro_ui.last_visit')}: {formatRelativeDate(client.lastOrderDate)}</p>
                  </div>
                </div>
                <div className="bg-primary-500/10 text-primary-600 px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest border border-primary-500/20">{client.totalCommandes || 0} CMD</div>
              </div>
              <div className="flex items-center gap-4 text-text-muted">
                <div className="flex items-center gap-1.5 text-xs font-bold"><Phone size={14} /> {getClientPhone(client)}</div>
                <div className="flex items-center gap-1.5 text-xs font-bold truncate"><Mail size={14} /> {client.email || '—'}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {totalPages > 1 && (
          <div className="p-6 bg-background/30 border-t border-border/50 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-xs font-black text-text-muted uppercase tracking-widest">{currentPage} {t('admin.pro_ui.page_of')} {totalPages} {t('admin.pro_ui.pages')} • {clients?.length} {t('admin.nav.clients').toLowerCase()}</p>
            <div className="flex gap-2">
              <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="px-4 py-2 rounded-xl bg-surface border border-border/50 text-xs font-black uppercase tracking-widest disabled:opacity-30 active:scale-95 transition-all">{t('admin.clients.pagination.prev')}</button>
              <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="px-4 py-2 rounded-xl bg-surface border border-border/50 text-xs font-black uppercase tracking-widest disabled:opacity-30 active:scale-95 transition-all">{t('admin.clients.pagination.next')}</button>
            </div>
          </div>
        )}
      </div>

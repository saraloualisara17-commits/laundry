      ...
      <div className={`${(isDetailView || isAddingUser) ? 'hidden md:flex' : 'flex'} md:w-72 lg:w-96 flex-col bg-surface rounded-3xl shadow-card border border-border/50 overflow-hidden shrink-0`}>
        <div className="p-5 lg:p-6 border-b border-border space-y-4 bg-background/30">
          ...
          <div className="relative group">
            ...
            <input type="text" placeholder={t('common.search_placeholder')} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full bg-background border border-border/50 rounded-xl py-2 text-sm font-bold text-text-primary outline-none focus:ring-4 focus:ring-primary-500/5 transition-all" />
          </div>
              ...
              {!isAddingUser && (
                <div className="mt-12 pt-8 border-t border-border/50">
                  <div className="flex items-center gap-2 mb-6">
                    ...
                    <h4 className="text-sm font-black text-text-primary uppercase tracking-widest">{t('admin.pro_ui.security_access')}</h4>
                  </div>
                  <form onSubmit={handleChangePassword} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="relative">
                        <input type={showPassword ? 'text' : 'password'} placeholder={t('admin.users.new_password')} value={newPassword} onChange={e => setNewPassword(e.target.value)} className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm font-bold text-text-primary outline-none focus:border-primary-500 transition-all shadow-sm" />

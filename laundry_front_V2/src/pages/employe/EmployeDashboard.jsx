...
import {
  ...
  Inbox, History, CalendarDays, LayoutList, Sun, Hash, TableProperties
} from 'lucide-react';
...
const STATUS_CONFIG = {
  [COMMANDE_STATUS.EN_ATTENTE]: { label: 'status.en_attente', icon: Clock, accentText: 'text-orange-500', accentBg: 'bg-orange-50 dark:bg-orange-500/10' },
  [COMMANDE_STATUS.EN_TRAITEMENT]: { label: 'status.en_traitement', icon: Wrench, accentText: 'text-blue-500', accentBg: 'bg-blue-50 dark:bg-blue-500/10' },
  [COMMANDE_STATUS.PRETE]: { label: 'status.prete', icon: CheckCircle2, accentText: 'text-green-500', accentBg: 'bg-green-50 dark:bg-green-500/10' },
  [COMMANDE_STATUS.RETOURNEE]: { label: 'status.retournee', icon: AlertTriangle, accentText: 'text-red-500', accentBg: 'bg-red-50 dark:bg-red-500/10' }
};
...
export default function EmployeDashboard() {
  ...
  const { list: commandes } = useSelector(selectCommandes);
  const loading = useSelector(selectLoading);

  const [searchTerm, setSearchTerm] = useState('');
  const [activeFilter, setActiveFilter] = useState(null);
  const [showAll, setShowAll] = useState(false);
  const [viewMode, setViewMode] = useState('active'); // active, handover
  ...
  const isPastUnfinished = (order) => {
    ...
    const orderDate = new Date(order.dateCreation || order.createdAt).setHours(0,0,0,0);
    return orderDate < today && ![COMMANDE_STATUS.PRETE, COMMANDE_STATUS.LIVREE, COMMANDE_STATUS.PAYEE].includes(order.status);
  };

  const filteredOrders = useMemo(() => {
    let list = commandes || [];
    
    // 1. Role Filter (Production vs Handover)
    if (viewMode === 'active') {
      list = list.filter(o => ![COMMANDE_STATUS.LIVREE, COMMANDE_STATUS.PAYEE].includes(o.status));
    } else {
      list = list.filter(o => [COMMANDE_STATUS.LIVREE, COMMANDE_STATUS.PAYEE].includes(o.status));
    }
    ...
    // 2. Search
    if (searchTerm) {
      list = list.filter(o => o.numeroCommande?.toLowerCase().includes(searchTerm.toLowerCase()));
    }
    ...
    // 4. Date Filter (Unless Show All)
    if (!showAll) {
      const today = new Date().setHours(0,0,0,0);
      list = list.filter(o => {
        const d = new Date(o.dateCreation || o.createdAt).setHours(0,0,0,0);
        return d === today || isPastUnfinished(o);
      });
    }

    return [...list].sort((a,b) => new Date(b.dateCreation || b.createdAt) - new Date(a.dateCreation || a.createdAt));
  ...
  const stats = useMemo(() => {
    ...
    return [
      ...
      { key: COMMANDE_STATUS.PRETE, label: t('workshop.stats.ready'), count: base.filter(o => o.status === COMMANDE_STATUS.PRETE).length, filterValue: COMMANDE_STATUS.PRETE },
    ];
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-16 px-4 md:px-0 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mt-2">
        <div className="text-start">
          <div className="flex items-center gap-3 mb-1">
            <h1 className="text-2xl md:text-3xl font-black text-text-primary uppercase tracking-tight">{t('workshop.title')}</h1>
           ...
           <div className="flex bg-surface p-1.5 rounded-2xl border border-border/50 shadow-sm w-full sm:w-auto">
             <button
               ...
               className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                 viewMode === 'active' ? 'bg-primary-600 text-white shadow-lg shadow-primary-500/20' : 'text-text-muted hover:text-text-primary hover:bg-background/50'
               }`}
             >
               <Inbox size={14} strokeWidth={2.5} /> {t('admin.nav.production')}
             </button>
             <button
               onClick={() => { setViewMode('handover'); setActiveFilter(null); }}
               className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                 viewMode === 'handover' ? 'bg-primary-600 text-white shadow-lg shadow-primary-500/20' : 'text-text-muted hover:text-text-primary hover:bg-background/50'
               }`}
             >
               <History size={14} strokeWidth={2.5} /> {t('admin.nav.sorties')}
             </button>
           </div>
          ...
          {stats.map(stat => {
            ...
            return (
              <div
                ...
                className={`bg-surface rounded-[2rem] shadow-card p-6 md:p-8 text-start border cursor-pointer hover:shadow-xl transition-all group overflow-hidden relative ${
                  isActive ? 'border-primary-500 ring-2 ring-primary-500/20 scale-[1.02]' : 'border-border/50 hover:border-primary-300'
                }`}
              >
                ...
                <div className="flex items-start justify-between mb-6 ps-2">
                   ...
                   <span className="text-4xl md:text-5xl font-black text-text-primary tracking-tighter leading-none">{stat.count}</span>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8">
           ...
           <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex gap-3">
                 ...
                 <button
                   ...
                   className={`px-6 py-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest flex items-center gap-2.5 transition-all shadow-sm whitespace-nowrap border ${
                     showAll 
                 ...
                 >
                   {showAll ? <><CalendarDays size={16} strokeWidth={2.5} />{viewMode === 'handover' ? t('workshop.pro_ui.today_handover') : t('workshop.table.view_today')}</> 
                            : <><LayoutList size={16} strokeWidth={2.5} />{viewMode === 'handover' ? t('workshop.pro_ui.handover_history') : t('workshop.table.view_all')}</>}
                 </button>
              </div>
           </div>

           {/* LIST/TABLE CONTAINER */}
           <div className="bg-surface rounded-[2rem] shadow-card border border-border/50 overflow-hidden text-start">
              <div className="px-6 md:px-8 py-5 border-b border-border/50 bg-background/30 flex items-center justify-between">
                 <h2 className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] flex items-center gap-2">
                    {viewMode === 'handover' ? t('workshop.pro_ui.handover_registry') : (showAll ? t('workshop.table.all_orders') : t('workshop.table.today_orders'))}
              ...
              {loading?.commandes && filteredOrders.length === 0 ? (
                ...
                <div className="py-24 px-6 text-center opacity-40">
                  ...
                  <h3 className="text-xl font-black text-text-primary uppercase tracking-tight mb-2">
                    {viewMode === 'active' ? t('workshop.table.no_orders_today') : t('workshop.pro_ui.empty_registry')}
                  </h3>
                        ...
                        {filteredOrders.map(order => {
                          ...
                          return (
                            <tr key={order.id} className={`hover:bg-background/40 transition-colors group relative ${isOverdue ? 'bg-red-500/[0.02]' : ''}`}>
                              <td className="px-8 py-6 relative text-start">
                              ...
                              <td className="px-8 py-6 text-sm font-black text-text-primary uppercase tracking-tight text-start">{order.commandeTapis?.length || 0} Unités</td>
                              ...
                              <td className="px-8 py-6 text-end">
                                <button onClick={() => navigate(`/employe/commandes/${order.id}`)} className="inline-flex items-center gap-2.5 bg-surface border border-border/50 text-primary-600 rounded-xl px-5 py-2.5 text-[10px] font-black uppercase tracking-widest hover:bg-primary-600 hover:text-white hover:border-primary-600 transition-all shadow-sm active:scale-95 group/btn">
                                  {viewMode === 'handover' ? t('workshop.pro_ui.verify') : t('workshop.table.manage_btn')} <ChevronRight size={14} strokeWidth={3} className="rtl:rotate-180 group-hover/btn:translate-x-0.5 transition-transform" />
                                </button>
                              </td>
                            </tr>
                          );
        ...
        <div className="hidden lg:block lg:col-span-1 shrink-0 text-start">
          <div className="bg-surface rounded-[2rem] shadow-card p-8 sticky top-[88px] border border-border/50 max-h-[calc(100vh-140px)] overflow-y-auto">
                    ...
                    <div key={order.id} className="relative ps-12 group cursor-pointer hover:bg-background/40 p-2 -my-2 rounded-xl transition-colors" onClick={() => navigate(`/employe/commandes/${order.id}`)}>
                      ...
                      <div className="min-w-0 pt-0.5">
                        ...
                        <p className="text-[10px] text-text-muted font-bold uppercase tracking-widest leading-relaxed">
                           {t('workshop.activity.transition', { status: t(cfg.label) })}
                        </p>
                      </div>
                    </div>
                  );

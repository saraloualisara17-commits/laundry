...
export default function RegisterClient() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-32 animate-fade-in px-4">
      ...
      {/* SEARCH SECTION */}
      <div className="bg-white rounded-2xl shadow-card p-2 flex items-center gap-3 border border-border/60 text-start">
        <div className="w-10 h-10 bg-primary-50 rounded-xl flex items-center justify-center text-primary-500 ms-1 shrink-0 border border-primary-100/50">
        ...
        <input 
          ...
          className="flex-1 py-3 text-sm font-semibold placeholder:font-medium placeholder:text-text-muted outline-none bg-transparent"
        />
        <button 
          onClick={() => alert(t('driver.register_client.scanner_unavailable'))}
          className="bg-gray-50 hover:bg-gray-100 rounded-xl px-5 py-2.5 text-xs font-bold text-text-secondary transition-colors flex items-center gap-2 border border-border/50"
        >
          <QrCode size={16} />
      ...
      {/* SEARCH RESULT CARD */}
      {searchResult && (
        <div className="border-2 border-primary-500 rounded-2xl p-6 mb-8 bg-white flex flex-col sm:flex-row items-center gap-6 shadow-xl animate-in zoom-in-95 duration-300">
          <div className="w-16 h-16 rounded-2xl bg-primary-50 text-primary-600 flex items-center justify-center shrink-0 border border-primary-100">
          ...
          <div className="flex-1 text-center sm:text-start min-w-0">
             <div className="flex flex-col sm:flex-row items-center gap-2 mb-2">
                ...
                <span className="bg-teal-50 text-teal-700 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider border border-teal-100">
      ...
      {!searchResult && searchPhone.length >= 8 && !loading.search && (
        <div className="bg-amber-50 border border-amber-100 rounded-xl py-4 px-6 text-center animate-in fade-in duration-300">
          <p className="text-xs font-bold text-amber-700 uppercase tracking-widest flex items-center justify-center gap-2">
      ...
      {/* NEW CLIENT FORM SECTION */}
       <div ref={formRef} className={`space-y-6 transition-all duration-700 text-start ${showForm ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}>
        ...
        {/* INFO BANNER */}
        <div className="bg-blue-50/50 border border-blue-100 rounded-2xl px-6 py-4 flex items-center gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-card border border-border/60 overflow-hidden">
             <div className="bg-gray-50/50 px-6 py-4 border-b border-border/50 flex items-center gap-3">
             ...
             <div className="p-6 sm:p-8 space-y-6">
                <div className="space-y-2">
                   ...
                   <input 
                     ...
                     className="w-full bg-gray-50 border border-border rounded-xl px-5 py-3.5 text-sm font-semibold focus:bg-white focus:border-primary-400 outline-none transition-all h-[52px]"
                   />
                </div>
             ...
             <div className="p-6 sm:p-8 space-y-6">
                <div className="bg-primary-50/50 border border-primary-100 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                   <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-white border border-primary-100 flex items-center justify-center text-primary-500 shadow-sm shrink-0">
                       ...
                       <div>
                         ...
                         <p className={`text-sm font-bold ${formData.latitude ? 'text-primary-700' : 'text-text-muted'}`}>
                   ...
                   <button 
                     ...
                     disabled={isLocating}
                     className="w-full sm:w-auto text-primary-600 text-xs font-bold uppercase tracking-wider hover:text-primary-700 bg-white px-4 py-2 rounded-lg shadow-sm border border-primary-100 active:scale-95 transition-all"
                   >
                ...
                <div className="space-y-1.5">
                   ...
                   <textarea 
                     ...
                     className="w-full bg-gray-50 border border-border rounded-xl px-4 py-3.5 text-sm font-semibold focus:bg-white focus:border-primary-400 outline-none transition-all resize-none min-h-[100px]"
                   />
                </div>
             </div>
          </div>
        ...
        <div className="pt-6 w-full pb-32 md:pb-0">
          ...
          <div className="md:hidden fixed bottom-[64px] pb-safe start-0 end-0 z-[110] flex justify-center px-4 sm:px-6">
             <div className="w-full max-w-5xl bg-white/95 backdrop-blur-lg border-t border-x border-border/60 rounded-t-2xl shadow-[0_-12px_40px_rgba(0,0,0,0.08)] py-3 md:py-4 px-5">
                <button
                  onClick={handleSubmit}
                  disabled={loading.createClient}

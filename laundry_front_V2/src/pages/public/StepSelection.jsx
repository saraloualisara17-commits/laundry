    ...
    <div className="max-w-6xl mx-auto px-4 md:px-10 py-6 md:py-10" dir="ltr">
      <div className="flex flex-col lg:flex-row gap-6 items-start">
            ...
            {categories.map(cat => {
              ...
              return (
                <button key={cat.id} onClick={() => setActiveCat(cat)}
                  className={`flex-shrink-0 px-5 py-2 rounded-full text-sm font-semibold transition-all duration-200 cursor-pointer border
                    ${active
                ...
                <div key={product.id}
                  ...
                  <div className="flex items-start gap-3">
                    <div className="w-20 h-20 rounded-xl overflow-hidden bg-gray-100 flex-shrink-0">
                      {product.imageUrl
                        ? <img src={imgSrc(product.imageUrl)} alt={prodName(product)} className="w-full h-full object-cover" />
                        : <div className="w-full h-full flex items-center justify-center text-3xl">🧺</div>
                      }
                    </div>
                    <div className="flex-1 min-w-0 pt-1">
                      <div className="flex items-baseline gap-2 flex-wrap">
                        <span className="text-sm font-bold text-[#1a2e44]">{prodName(product)}</span>
                        {product.prixUnitaire && (
                          <span className="text-sm font-bold text-[#0D7377]">
                            {product.prixUnitaire} dh
                            <span className="text-xs font-normal text-gray-400">
                              /{product.requiresDimensions ? 'm²' : t('public.unit', { defaultValue: 'u' })}
                            </span>
                          </span>
                        )}
                      </div>
        ...
        <div className="hidden lg:block w-72 flex-shrink-0 sticky top-24">
          <OrderSummaryCard
            totalItems={totalItems} totalDh={totalDh}
            onNext={onNext} t={t}
          />
        </div>
      </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-100 shadow-2xl px-4 py-3">
        {/* Expandable summary */}
        {summaryOpen && (
          <div className="mb-3 bg-gray-50 rounded-2xl p-4 flex flex-col gap-2">
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">{t('public.items_selected', { defaultValue: 'Items Selected' })}</span>
              <span className="font-semibold text-[#1a2e44] tabular-nums">{totalItems}</span>
            </div>
            <div className="flex justify-between text-sm border-t border-gray-200 pt-2">
              <span className="font-bold text-[#1a2e44]">{t('public.total', { defaultValue: 'Total' })}</span>
              <span className="font-extrabold text-[#0D7377] tabular-nums">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setSummaryOpen(o => !o)}
            ...
            <div className="flex items-center gap-1">
              <span className="text-base font-extrabold text-[#1a2e44] tabular-nums">
                {totalDh > 0 ? `${totalDh.toFixed(0)} dh` : '—'}
              </span>
              {totalItems > 0 && (
                <span className="text-xs text-gray-400">· {totalItems} {t('public.unit', { defaultValue: 'u' })}</span>
              )}
          ...
          <button onClick={onNext} disabled={totalItems === 0}
            className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm transition-all
              ${totalItems > 0
                ? 'bg-[#1a2e44] text-white cursor-pointer shadow-lg shadow-slate-900/20'
                : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
            {t('public.nav_info', { defaultValue: 'Continue' })}
...
function OrderSummaryCard({ totalItems, totalDh, onNext, t }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between py-3 border-b border-gray-100">
        <span className="text-sm text-gray-500">{t('public.items_selected', { defaultValue: 'Items Selected' })}</span>
        <span className="text-sm font-semibold text-[#1a2e44] tabular-nums">{totalItems}</span>
      </div>
      <div className="flex items-center justify-between py-1">
        <span className="text-sm text-gray-500">{t('public.service_fee', { defaultValue: 'Service Fee' })}</span>
        <span className="text-sm font-semibold text-[#1a2e44] tabular-nums">
          {totalDh > 0 ? `${totalDh.toFixed(0)} dh` : '0 dh'}
        </span>
      </div>
      <div className="border-t border-gray-100 pt-3 flex items-center justify-between">
        <span className="text-base font-extrabold text-[#1a2e44]">{t('public.total', { defaultValue: 'Total' })}</span>
      ...
      <p className="text-[11px] text-gray-400 text-center leading-relaxed">
        {t('public.terms_note', { defaultValue: 'By proceeding, you agree to our terms of service.' })}
      </p>
      <div className="flex items-start gap-2.5 bg-[#f0faf0] rounded-xl px-3.5 py-3">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
          stroke="#3DB23D" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0 mt-0.5">
          <path d="M2 22c1-1 1-4 4-4h3c2 0 4-1 4-4V3s-3 0-5 2-3 4-3 7"/>
          <path d="M6 18c0-6 3-10 8-12"/>
        </svg>
        <div>
          <p className="text-xs font-bold text-[#3DB23D]">{t('public.eco_choice', { defaultValue: 'Eco-Choice' })}</p>
          <p className="text-[11px] text-[#3DB23D]/70">{t('public.eco_desc', { defaultValue: 'Using 100% biodegradable detergents' })}</p>
        </div>
      </div>
    </div>
  );
